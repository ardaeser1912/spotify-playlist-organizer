"""MockClient — fixtures üzerine SpotifyClient sözleşmesini uygular.

Tüm testler ve servis-testleri bunu kullanır (gerçek API YOK). Yazma işlemleri
(create/add/replace) bellekte kaydedilir ki apply testleri doğrulayabilsin.
Dönen track'ler KOPYADIR → testler fixtures'ı bozamaz.
"""
...
class MockClient:
    def __init__(self):
        self.created: list[dict] = []          # [{id, name, uris}]
        self.added: dict[str, list[str]] = {}  # playlist_id -> uris
    ...
    def top_tracks(self, limit: int = 20) -> list[dict]:
        tracks = sorted(fixtures.TRACKS.values(),
